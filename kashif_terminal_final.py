...
# ===== کلیدهای API (کلیدها را با کلید واقعی خود جایگزین کنید) =====
...
# ===== ایجاد دیتابیس =====
conn = sqlite3.connect("kashif_ip_mining_scan.db")
cursor = conn.cursor()
cursor.execute('''
...
def generate_html_report():
    ...
    with open("kashif_report.html", "w", encoding="utf-8") as f:
        f.write("<html><head><meta charset='utf-8'><title>گزارش اسکن ماینر</title></head><body>")
        f.write("<h1>گزارش اسکن IP ماینر (KASHIF)</h1><table border=1><tr><th>IP</th><th>ProxyCheck</th><th>AbuseIPDB</th><th>Shodan</th><th>IPInfo</th><th>Open Ports</th><th>Timestamp</th></tr>")
        for row in rows:
            f.write("<tr>")
            for col in row:
...
def main():
    print("=======================================")
    print("     🚨 کاشف - شبح حبشی 🚨")
    print("     سیستم کشف ماینر غیرمجاز")
    print("=======================================")
    print("ابتدا رنج IP را وارد کنید (مثال: 192.168.1.1-192.168.1.255)")
    ip_range = input("رنج IP: ").strip()
    try:
    ...
    print("\n✅ اسکن کامل شد. گزارش در فایل kashif_report.html ذخیره شد.")
    generate_html_report()
    print("برای مشاهده گزارش، فایل kashif_report.html را باز کنید.")
